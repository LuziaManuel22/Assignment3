 ...
 int factArr[21]; // Factorial Array
         ...
         if (ret_val >= 0 ) {
             ...
             for (i=1;i < MAX_CONNECTIONS;i++) {
                 if ((all_connections[i] > 0) &&
                     ...
                     int recv_val;
                     ret_val = recv(all_connections[i], &recv_val, sizeof(int), 0);
                     if (ret_val == 0) {
                     ...
                     if (ret_val > 0) { 
                         printf("Received data (len %d bytes, fd: %d): %s\n", 
                            ...
                            int fact_n = fact(recv_val);
                            ret_val = send(all_connections[i], &fact_n, sizeof fact_n, 0);
                            if(ret_val == -1) {
